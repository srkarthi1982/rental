...
import type { RentalDashboardSummaryV1 } from "../dashboard/summary.schema";
...
export const pushRentalActivity = async (params: {
  userId: string;
  activity: RentalActivity;
  summary: RentalDashboardSummaryV1;
}): Promise<void> => {
  try {
    ...
    if (import.meta.env.DEV) {
      console.warn("pushRentalActivity failed", error);
    }
  }
};
